...
def build_base_smt(printable: bool) -> tuple[list[str], list[str]]:
...
def solve_with_model_blocking(printable: bool, payload: bytes, main_bytes: bytes, max_models: int = 200) -> bytes | None:
    vars_, base_lines = build_base_smt(printable)
    blocking_clauses: list[str] = []
    crc = binascii.crc32(main_bytes) & 0xFFFFFFFF

    for _ in range(max_models):
        lines = list(base_lines)
        lines.extend(blocking_clauses)
        ...
        if len(plain) >= 4 and plain[:4] == b"PK\x03\x04":
            return solved

        neq = " ".join(f"(not (= c{i} #x{b:02x}))" for i, b in enumerate(solved))
        blocking_clauses.append(f"(assert (or {neq}))")
...
def main() -> int:
    ...
    solved = solve_with_model_blocking(printable=True, payload=payload, main_bytes=main_bytes)
    if solved is None:
        solved = solve_with_model_blocking(printable=False, payload=payload, main_bytes=main_bytes)
    if solved is None:
